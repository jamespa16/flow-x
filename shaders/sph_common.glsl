/* Shared prelude for every Phase 4 SPH compute pass.
 *
 ...
 * so the helpers below can be written once. The block is 112 bytes, under the
 * 128-byte push-constant budget Blender's backends guarantee - keep it that
 * way when adding parameters (pack into the spare lanes instead).
 *
