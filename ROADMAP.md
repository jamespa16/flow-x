...
| GPU compute path | ~~Blender's `gpu` module + GLSL~~ **Own Metal device** via a native helper dylib (`native/`, built into `bin/`), with a numpy CPU fallback engine | Replaced post-MVP (unreleased v0.3): Blender's Metal implementation blocked features the whitewater work needed (image atomics, image arrays), and the workarounds cost more than owning the device. `gpu` remains only for viewport drawing. CUDA is the next backend |
| Solver algorithm | ~~WCSPH~~ **PBF** (position-based fluids) | Replaced WCSPH: stiffness came from projection iterations instead of a Tait EOS, so the timestep is no longer stiffness-limited |
| Neighbor search | Uniform grid spatial hash, built on GPU each step; cells sorted with a bitonic sort | Kept from the original design. Its original reason (no image atomics on Blender's Metal) is gone — atomics work on the own-device backend — but the sort is not yet rewritten as a counting sort (see `scripts/golden.py` before touching it) |
